...
export const useAiStore = defineStore('ai', {
  ...
  actions: {
    ...
    // 3. Rekomendasi Lahan (Random Forest)
    async predictCropRecommendation(N, P, K, temperature, humidity, ph, rainfall) {
      this.isLoading = true
      this.error = null
      this.predictionResult = null

      try {
        // Payload JSON berisi 7 parameter Float
        const payload = {
          N: N,
          P: P,
          K: K,
        ...
        const url = 'http://127.0.0.1:8080/rf/predict'
        const response = await axios.post(url, payload)
        ...
        console.error("Gagal menganalisis lahan:", err)
        this.error = err.response?.data?.detail || "Terjadi kesalahan saat menghubungi server AI."
        return false
      } finally {
        this.isLoading = false
